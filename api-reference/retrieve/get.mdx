...
api: 'POST /retrieve'
description: 'Fetch the most relevant chunks of data from any Source within a specified Zone based on a query'
authMethod: "bearer"
---

This endpoint offers powerful search capabilities with optional features for fine-tuning results. It retrieves and ranks relevant documents from a specified Zone.
...
<ParamField body="expandQuery" type="boolean" default="true">
...
<ParamField body="zoneId" type="string">
  The unique identifier of the zone to search in. Required if zoneReferenceId is not provided.
</ParamField>

<ParamField body="zoneReferenceId" type="string">
  The reference ID of the zone to search in. Required if zoneId is not provided.
</ParamField>
...
-d '{
  ...
  "rerank": true,
  "expandQuery": false,
  "zoneId": "6696387fb71bf922119a374f"
}'
```
</RequestExample>
...
- HTTP 500: Returned if there's a server error during retrieval.

<Note>
  Either `zoneId` or `zoneReferenceId` must be provided. Also, either `content` or `audio` is required, but not both.
</Note>